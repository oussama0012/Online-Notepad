class NotepadApp {
    ...
    handleToolbarAction(action) {
        switch(action) {
            ...
            case 'bold':
                ...
                break;
            case 'italic':
                this.applyFormattingWithSelectionPreserved('italic');
                break;
            case 'underline':
                this.applyFormattingWithSelectionPreserved('underline');
                break;
            case 'emoji':
    ...
    applyFormattingWithSelectionPreserved(command) {
        ...
        if (range.collapsed) {
            // Create a temporary span if no text is selected (cursor only)
            const tempSpan = document.createElement('span');
            tempSpan.textContent = '\u200B'; // Zero-width space
            
            range.insertNode(tempSpan);
            
            // Apply style to the temporary span
            switch(command) {
                case 'bold':
